...
if __name__=="__main__":
    ...
    parser.add_argument('--start_profile',action="store_true")
    parser.add_argument('--stop_profile',action="store_true")
    parser.add_argument('--export_timeline',action="store_true")
    parser.add_argument('--port',type=str,default=5600)
    args = parser.parse_args()
    port = args.port
    url=f"{url}:{port}"
    if args.start_profile:
        asyncio.run(async_request_openai_completions(start_profile_input()))
    if args.stop_profile:
        asyncio.run(async_request_openai_completions(end_profile_input()))
    if args.export_timeline:
        asyncio.run(async_request_openai_completions(export_timeline()))