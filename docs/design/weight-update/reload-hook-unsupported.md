...
| 4 | FP8 block-wise：UE8M0 requant（SM100 DeepGemm，`VLLM_USE_DEEP_GEMM_E8M0=1`） | 抛异常 | requant 会改写权重本体，需要权重 CONVERT buffer |
...
| 6 | FP8 block-wise：Marlin / Humming / B12x / XPU / CPU / ROCm(AITER) 后端 | 抛异常 | 非 CUDA 主线后端，layout 各异，按需逐个补齐 |
