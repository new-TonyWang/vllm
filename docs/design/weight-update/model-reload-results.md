...
| `Qwen3-30B-A3B-FP8` | day0-kit NCCL | **PASS** | `day0-qwen30-fp8-real-nccl-rerun.json`; DeepGEMM linear and TRITON FP8 MoE selected; an independent runtime probe enumerated `model.layers.*.mlp.experts` and `routed_experts` with `quant_method=Fp8MoEMethod`, module `vllm.model_executor.layers.quantization.fp8`, `block_quant=true`, and `fp8_backend=Fp8MoeBackend.TRITON`; NCCL 2.28.9 rank 0 / GPU 1 / nranks 2 Init COMPLETE; 37,491 tensors, 32,444,792,832 bytes, 52 buckets; verifier PASS; H200 jobs `e17734c6a0f2` and class probe `77124`, `rc=0` |
| `Qwen3-30B-A3B-FP8` experimental linear refresh | day0-kit NCCL | Protocol PASS; selective contract NOT VERIFIED | `day0-qwen30-fp8-selective-update.json`; block-FP8 checkpoint, not per-tensor; refresh still invokes PWAL in `3208cd74ad`. Generation `day0-0ed8ec2d62354a60890ec7478f36471d`; 37,491 tensors, 32,444,792,832 bytes, 52 buckets; protocol verifier PASS; H200 `rc=0`. Inspected pointers cover embedding/norm/head, not quantized linear runtime state. |
| `Qwen2.5-1.5B-Instruct-AWQ` | day0-kit NCCL | **PROTOCOL PASS; AWQ GEMM selective behavior PASS** | `day0-awq-selective-update.json`; AWQ GEMM INT4, group size 128, 731 tensors, 1,614,472,192 bytes, 4 buckets; generation `day0-bf418248c1764f02af5df116aa1a3b6e`; strict verifier PASS; H200 `rc=0`. The focused test separately disables PWAL after cold load and verifies two reloads preserve qweight/qzeros/scales values and addresses. |
