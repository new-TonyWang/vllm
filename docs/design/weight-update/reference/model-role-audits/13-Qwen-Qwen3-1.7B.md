...
- 架构参数: 28 层,hidden 2048,16 Q 头 / 8 KV 头,head_dim=128,`tie_word_embeddings=true`
...
| `lm_head`(tie → embed_tokens 别名) | qwen3.py:297-299;skip_prefixes qwen3.py:341 | R1(别名) | P3(单份) | RESTORABLE | 低;同一存储勿双写 |
...
1. `_ROPE_DICT` 全局单例(rotary_embedding/__init__.py:30,83-84,383),R6:跨引擎实例共享同一 `cos_sin_cache`。
2. `cos_sin_cache` persistent=False:当前 P2(`named_buffers()`)可存;基于 `state_dict()` 的实现会漏。终态 RECOMPUTE(base.py:94-103 可重算)。
3. `q_range/k_range/v_range` 裸张量缺口:默认无害,fp8 kv 动态 scale 场景踩雷。
4. tie 词嵌入:`lm_head` 与 `embed_tokens` 同一 Parameter(qwen3.py:299);`load_weights` skip `lm_head.`(qwen3.py:341);reload 元数据在模型 init 后立即记录(model_loader/utils.py:64),别名关系保持一致。
5. bf16 无量化 → 无 R3/R4/R5 状态。

## 结论

Qwen3-1.7B(bf16)状态结构与 0.6B 相同(tie 词嵌入、纯 R1 + 两组 R2);sleep-L2 依赖 P2 恢复 `cos_sin_cache` 与 attention scales,reload 依赖 P3 覆写全部 R1。唯一缺口是 `q_range/k_range/v_range` 裸张量(默认路径无害)。
